    ...
    try {
      ...
      if (Object.keys(query).length) {
        u +=
          ...
          Object.keys(query)
            .map((attr) =>
              query[attr] === undefined ? attr : attr + '=' + query[attr]
            )
            .join('&')
      }
      ...
      if (this.connected) {
        ...
        handlers.error &&
          handlers.error.forEach((cb) =>
            cb.call(this, ERRORS[error.code] || 'UNKNOWN')
          )
      }
      this.close()
    }
    ...
    socket.onmessage = (message) => {
      ...
      if (type === MESSAGE_TYPES.CALLBACK) {
        ...
        if (name === '___ready___') {
          ...
          if (wasConnected) {
            handlers.reconnect &&
              handlers.reconnect.forEach((cb) => cb.call(this, true))
          } else {
            handlers.connect &&
              handlers.connect.forEach((cb) => cb.call(this, true))
            wasConnected = true
          }
          ...
          handlers[name] &&
            handlers[name].forEach((cb) =>
              cb.call(this, args[0], args[1], args[2], args[3], args[4])
            )
        } else {
    ...
    if (name === 'authenticate') {
      authTimeout = setTimeout(() => {
        ...
        if (this.connected) {
          ...
          handlers.error &&
            handlers.error.forEach((cb) =>
              cb.call(this, 'Authenticate timeout')
            )
        }
        this.close()
      }, options.authTimeout)
  ...
  this.emit = (name, arg1, arg2, arg3, arg4, arg5) => {
    ...
    try {
      if (typeof arg5 === 'function') {
        ...
        socket.send(
          JSON.stringify([MESSAGE_TYPES.MESSAGE, id, name, [arg1, arg2]])
        )
      } else if (arg4 === undefined && arg5 === undefined) {
        socket.send(
          JSON.stringify([MESSAGE_TYPES.MESSAGE, id, name, [arg1, arg2, arg3]])
        )
